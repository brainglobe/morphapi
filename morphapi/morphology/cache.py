...
class NeuronCache(Paths):
    cache_filenames_parts = ['_soma', '_axon', '_apical_dendrites', '_basal_dendrites', 'whole_neuron']
    ...
    def get_cache_filenames(self, neuron_name):
        ...
        if not os.path.isdir(fld):
            os.mkdir(fld)
        return  [os.path.join(fld, str(neuron_name)+part+'.obj') for part in self.cache_filenames_parts]
    ...
    def load_cached_neuron(self, neuron_name, _params):
        ...
        # Load neurites
        neurites = ['soma', 'axon', 'apical_dendrites', 'basal_dendrites', 'whole_neuron']
        loaded =  {nn:load(fp) for nn, fp in zip(neurites, self.get_cache_filenames(neuron_name))}
    ...
    def write_neuron_to_cache(self, neuron_name, neuron, _params):
        ...
        if isinstance(neuron, Mesh):
            ...
            if not isinstance(neuron, dict):
                raise ValueError(f'Invalid neuron argument passed while caching: {neuron}')
            for key, actor in neuron.items():
                if key == 'whole_neuron': 
                        ...
                        if isinstance(actor, (list, tuple)):
                            if len(actor) == 1:
                                ...
                                try:
                                    ...
                                    raise ValueError(f'{key} actor should be a mesh or a list of 1 mesh not {actor}')
                    
                    if actor is None: actor = Mesh()
                    ...
                    if fname:
                        ...
                        raise ValueError(f'No filename found for {key}. Filenames {file_names}')
